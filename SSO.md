...
|界面|触发条件|提示方式|提示内容|
|:--:|:--:|:--:|:--:|
|验证码登录、找回密码、注册界面|输入手机号的格式不正确|在内容下方红色字体显示|手机号格式不正确|
|验证码登录、找回密码|输入的手机号未注册|在内容下方红色字体显示|该手机号未注册|
|验证码登录、找回密码、注册界面|滑块滑动到最右边|滑块内部显示|验证成功|
|验证码登录、找回密码、注册界面|点击发送验证码按钮|toast|验证码已发送到手机，请查收|
|验证码登录、找回密码、注册界面|在有效时间内，短信验证码填写不正确|在内容下方红色字体显示|验证码错误|
|验证码登录、找回密码、注册界面|验证码过期之后用户才输入该验证码|toast|验证码已失效，请点击重新发送|
|帐号登录|输入的帐号不正确|在内容下方红色字体显示|帐号格式不正确|
...
|注册界面|输入的手机号已注册|在内容下方红色字体显示|该手机号已注册|
|注册界面|输入的昵称和其他人相同|在内容下方红色字体显示|该昵称已被使用|
|注册界面|输入的昵称格式不正确|在内容下方红色字体显示|昵称格式不正确|
|所有界面|网络状况不好，进行需要网络的操作|toast|网络不给力|

### 1.3 页面流图
#### 1.3.1 APP端页面流图
![avatar](../SSO/img/p1_3_1.png)
#### 1.3.2 WEB端页面流图
![avatar](../SSO/img/p1_3_2.png)
